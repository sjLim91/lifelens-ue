...
| 쭌 + 쭌 AI | `jjun/belief-core-v1` | SPEC 30 기반 Belief Core v1 + Memory evidence 반영 + 테스트 | `Source/LifeLensCore/**` 중 Belief 전용 파일/테스트 | DOING |
...
- PR #4 Relationship Core v1, PR #6 Emotion Core v1, PR #7 Memory Core v1은 모두 Core CI + Preflight PASS 후 `main` 병합 완료.
- TASK_03 실행을 기다리지 않고 `jjun/belief-core-v1`에서 Memory → Belief 기반을 병렬 진행한다. TASK_03의 Bridge/Build 파일은 건드리지 않는다.
...
3. 별도 `jjun/belief-core-v1`에서 Memory evidence → Belief confidence/stance 누적 규칙과 테스트 구현.
...
- `jjun/belief-core-v1` Belief Core PR — 구현/코어 테스트 후 생성 예정.
