...
### ASSIST_LOCK-GFX-PCG-RUNTIME-1 — Jjun scoped PCG presentation assist

- explicit user direction: push visible graphics toward final completion.
- helper: Jjun.
- owner/reviewer: Dagyeom.
- branch: `integration/desktop-pcg-runtime-dressing-v1`.
- cross-owner files:
...
- scope:
  - consume the authored `PCG_LL_GroundCover` graph at runtime on desktop.
  - seed it from authoritative initial-chunk `VisualSeed`.
  - keep PCG decorative-only; no gameplay resource/facility authority.
  - Android does not spawn/link the PCG runtime path.
- Jjun-owned integration files: Build.cs, GameMode, uproject, CI.
- release after PR merge/close and Team Board reconciliation.
