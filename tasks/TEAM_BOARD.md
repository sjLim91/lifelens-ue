...
| 쭌 + 쭌 AI | `jjun/emotion-core-v1` | SPEC 25 기반 Emotion Core v1 + 사건/감쇠 규칙 + 테스트 | `Source/LifeLensCore/**` 중 Emotion 전용 파일/테스트 | DOING |
...
- TASK_03 실행을 기다리지 않고 `jjun/emotion-core-v1`에서 SPEC 25 Emotion 확장을 병렬 진행한다. TASK_03의 Bridge/Build 파일은 건드리지 않는다.
...
3. 별도 `jjun/emotion-core-v1`에서 Emotion 다차원 상태, 사건 입력, 시간 감쇠, 파생 valence/arousal 테스트 구현.
...
- `jjun/emotion-core-v1` Emotion Core PR — 구현/코어 테스트 후 생성 예정.
