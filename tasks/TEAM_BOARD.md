...
| 쭌 + 쭌 AI | `jjun/memory-core-v1` | SPEC 29~30 기반 Memory Core v1 + 신뢰도/감쇠/회상 점수 + 테스트 | `Source/LifeLensCore/**` 중 Memory 전용 파일/테스트 | DOING |
...
- PR #4 Relationship Core v1은 Core CI와 Preflight 모두 PASS 후 `main` 병합 완료.
- PR #6 Emotion Core v1도 Core CI와 Preflight 모두 PASS 후 `main` 병합 완료.
- TASK_03 실행을 기다리지 않고 `jjun/memory-core-v1`에서 SPEC 29~30 Memory 확장을 병렬 진행한다. TASK_03의 Bridge/Build 파일은 건드리지 않는다.
...
3. 별도 `jjun/memory-core-v1`에서 MemoryRecord 필드, 신뢰도/중요도/감쇠, 태그 기반 회상 점수 테스트 구현.
...
- `jjun/memory-core-v1` Memory Core PR — 구현/코어 테스트 후 생성 예정.
