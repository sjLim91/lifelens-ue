...
### 다겸 측 AI — IR-B 캐릭터 정면 축 수정

- 작성자: 다겸 측 AI
- 브랜치: `dagyeom/character-facing-axis-fix`
- 커밋: `caf7706`
- 상태: `화면 확인 완료 / 리뷰 대기`
- 보드 확인: `tasks/TEAM_BOARD.md`의 IR-B가 REOPENED이고 담당이 다겸, 주 의심점이 `MeshForwardYawOffsetDegrees = 90.0f`로 지정되어 있었다. Core/World 이동은 의심 대상이 아니라고 명시되어 있다.
- 원인 — 추측이 아니라 임포트된 스켈레톤의 컴포넌트 공간 좌표 측정으로 확정했다.
  - `ball_l`(발끝) Y=+11.3, `foot_l` Y=-3.6 → 메시 정면은 로컬 +Y
  - `hand_l` X=+73.9, `hand_r` X=-73.9 → 측면 축은 X
  - UE 액터·월드 정면은 +X이므로 +Y 정면을 진행 방향에 맞추려면 yaw -90도가 필요하다. 기존 +90은 정확히 반대로 돌렸다.
- A/B 검증 — `ll.DebugMotion` 로그에 `facing` 지표를 추가했다. 메시 정면(컴포넌트 right vector)을 진행 방향에 투영한 값이며 +1이 정방향, -1이 역방향이다.
  - `+90`: 이동 프레임 170건 전부 `facing=-1.00`
  - `-90`: 이동 프레임 116건 전부 `facing=1.00`
  - 중간에 빌드가 7초 만에 끝나 타임스탬프로 인한 미반영을 의심했고, 강제 재빌드 후 재검증했다.
- 화면 확인: 주민이 진행 방향을 보고 걷는다. 후진 보행 해소 확인.
- 이번 화면에서 개별 판정하지 못한 항목 (보드 요구 사항):
  - 직선 이동 방향
  - 정지 후 상호작용 대상 방향
  - 남녀 및 의상 변형별 방향
  - 필요하면 다음 사이클에서 하나씩 확인한다.
- 범위: Core/World 이동 권한과 액터 yaw는 변경하지 않았다. 표현용 바디 회전만 보정했다.
- 상대가 알아야 할 점:
...
### 다겸 측 AI — Observer 확인 완료와 IR-E 제출

- 작성자: 다겸 측 AI
- 브랜치/PR: `dagyeom/observer-readability-and-qa-view`, PR #98
- 커밋: `433fecc`
- 상태: `확인 완료 / IR-E 회신 대기`
- 확인됨:
  - `ll.ViewResidents` 정상 동작. 주민 4명 전원이 적절한 거리에서 잡히고 선택 링, 이름·생애단계 라벨, LEVEL 1 카드 모두 정상 표시. 이전 20 km 이탈 문제 해소
  - LEVEL 2 패널 불투명도 적정. 0.92 / 0.78 / 0.40 유지
  - CI: Preflight PASS, Core Tests PASS, Unreal Linux Compile PASS
- IR-E 제출 (카메라, 쭌 소유):
  - 재확인 결과 휠 줌과 회전은 동작한다
  - IR-E-1 초기 카메라 각도가 주민 활동 구역을 담지 못해 시작 직후 조작해야 주민을 찾을 수 있다. PR #100의 밀도 조정 이후에도 동일
  - IR-E-2 패닝이 없어 시야를 옆으로 옮길 수 없다
  - 다겸 측 완충안으로 초기 카메라 시선 방향의 큰 수목 추가 억제를 제시했다. presentation-only이며 임시 완충이라는 점을 명시했다
- 상대가 알아야 할 점:
  - `ll.ViewResidents`로는 주민이 정상적으로 잡히므로 월드 배치나 표현 문제가 아니라 초기 카메라 위치·각도 문제다
