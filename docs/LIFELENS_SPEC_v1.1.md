...
Core Simulation은 Android / Windows / macOS에서 동일하게 유지한다. PC와 Android는 그래픽 품질과 일부 성능 예산만 다르게 하며, Windows와 macOS는 같은 PC 기능 로드맵을 공유한다.
...
- Windows PC: DX12 + SM6 기반 Lumen GI/Reflections, Nanite eligible assets, Virtual Shadow Maps, TSR, 고품질 PBR/atmosphere를 적극 활용한다.
- macOS: PC 제품 타깃이자 정상적인 Unreal Editor/그래픽 검수 환경이다. broad baseline은 Lumen software GI/reflections + TSR + Mesh Distance Fields를 사용하고, UE 5.6에서 Beta인 Apple Silicon M2+ Nanite/VSM은 실제 장비 profiling 후 선택형 high tier로 올린다.
