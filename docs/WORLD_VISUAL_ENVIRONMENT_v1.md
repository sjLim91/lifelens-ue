...
- desktop runtime consumes the graph for the initial observer region using the authoritative initial-chunk `VisualSeed`. Broader chunk-by-chunk PCG expansion remains a later coverage/performance step; gameplay resource truth stays Core-owned.
