...
3. Existing Quaternius stylized assets remain fallback only.
4. No paid pack or subscription is required for runtime correctness.
...
Presentation should prefer:
`/Game/Environment/Photoreal/**`

and fall back to:
`/Game/Environment/Quaternius/**`

only when a photoreal asset has not yet been imported/cooked.

This lets source/CI stay functional while the asset library is upgraded in
waves instead of requiring one giant binary change.
