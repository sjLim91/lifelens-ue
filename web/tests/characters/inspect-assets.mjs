...
try {
 ...
 const profile=api.createResidentAppearanceProfile({id:'actual-asset-check',sex:'Female',ageYears:26});api.applyResidentMaterialVariant(model,profile);api.addResidentHairVariant(visual,profile);
 const hair=visual.getObjectByName('LifeLensResidentHairVariant');if(hair?.parent.name!=='Head')throw Error('actual rig hair attachment failed');
 ...
 console.log(JSON.stringify({modelSize:size.toArray(),hairParent:hair.parent.name,headDisplacement:first.distanceTo(next),bodyColorVertices:colors.count,walkDuration:walk.duration,rootTracks:walk.tracks.filter(t=>t.name.startsWith('root.')).map(t=>({name:t.name,first:[...t.values.slice(0,4)],last:[...t.values.slice(-4)]}))},null,2));
